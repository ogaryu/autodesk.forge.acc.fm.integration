...
namespace forgeSample.Controllers
{
    ...
    public class DesignAutomationController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("api/forge/fm_revit/sync_assets")]
        public async Task<IActionResult> SyncAssets([FromForm] InputData input)
        {
            JObject workItemData = JObject.Parse(input.data);

            JArray assetData = await FMDataController.GetAssetsFromFMDB(workItemData);
            ...
            try
            {
                ...
                string folderId = folderParams[folderParams.Length - 1];
                string projectId = folderParams[folderParams.Length - 3];

                // Get RVT file download url
                string[] idParams = inputRvtFileUrl.Split('/');
                string inputProjectId = idParams[1];
                string inputVersionId = idParams[2];

                VersionsApi versionApi = new VersionsApi();
                versionApi.Configuration.AccessToken = Credentials.TokenInternal;
                dynamic version = await versionApi.GetVersionAsync(inputProjectId, inputVersionId);
                dynamic versionItem = await versionApi.GetVersionItemAsync(inputProjectId, inputVersionId);
                ...
                string rvtFileStorageUrl = string.Format("https://developer.api.autodesk.com/oss/v2/buckets/{0}/objects/{1}", bucketKey, objectName);

                // prepare workitem arguments
                // 1. input file
                XrefTreeArgument inputRvtFileArgument = new XrefTreeArgument()
                {
                    Url = string.Format(rvtFileStorageUrl),
                ...
                {
                    Url = "data:application/json, " + ((JArray)assetData).ToString(Formatting.None).Replace("\"", "'")
                };

                // 3. output file
                string outputRVTFileStorageName = inputRvtFileName;

                dynamic storageCreatedRVT = await this.CreateStorage(folderId, projectId, outputRVTFileStorageName);

                string[] rvtFileStorageIdParams = ((string)storageCreatedRVT.data.id).Split('/');
                string[] rvtFileBucketKeyParams = rvtFileStorageIdParams[rvtFileStorageIdParams.Length - 2].Split(':');
                string rvtFileBucketKey = rvtFileBucketKeyParams[rvtFileBucketKeyParams.Length - 1];
                string rvtFileObjectName = rvtFileStorageIdParams[rvtFileStorageIdParams.Length - 1];
                string uploadRvtFileUrl = string.Format("https://developer.api.autodesk.com/oss/v2/buckets/{0}/objects/{1}", rvtFileBucketKey, rvtFileObjectName);

                XrefTreeArgument outputRvtFileArgument = new XrefTreeArgument()
                {
                    Url = uploadRvtFileUrl,
                    Verb = Verb.Put,
                    Headers = new Dictionary<string, string>()
                   {
                       {"Authorization", "Bearer " + userToken }
                ...
                string callbackUrl = string.Format("{0}/api/forge/fm_revit/sync_assets/callback?id={1}&rvtFileName={2}&rvtFileBucketKey={3}&rvtFileObjectName={4}&rvtStorageId={5}&projectId={6}&folderId={7}", Credentials.GetAppSetting("FORGE_WEBHOOK_URL"), browerConnectionId,outputRVTFileStorageName, rvtFileBucketKey, rvtFileObjectName, (string)storageCreatedRVT.data.id, projectId, folderId);
                ...
                {
                    { "inputRvtFile", inputRvtFileArgument },
                    { "inputJsonFile",inputJsonArgument },
                    { "resultRvtFile", outputRvtFileArgument },
        ...
        /// </summary>
        [HttpPost]
        [Route("api/forge/fm_revit/sync_assets/callback")]
        public async Task<IActionResult> OnCallbackSyncAssets(string id, string rvtFileName, string rvtFileBucketKey, string rvtFileObjectName, string rvtStorageId, string projectId, string folderId, [FromBody]dynamic body)
        {
            try
            {
                await CreateItemVersion(projectId, folderId, rvtFileName, rvtStorageId);
                ...
                outputData.RvtFileName = rvtFileName;
                outputData.RvtFileBucketKey = rvtFileBucketKey;
                outputData.RvtFileObjectName = rvtFileObjectName;

                var jsonData = JsonConvert.SerializeObject(outputData);

                await _hubContext.Clients.Client(id).SendAsync("onCompleteFMAssetsToRevit", jsonData);
        ...
        [HttpPost]
        [Route("api/forge/fm_revit/replace_family")]
        public async Task<IActionResult> ReplaceFamily([FromForm] InputData input)
        {
            JObject workItemData = JObject.Parse(input.data);
            ...
            try
            {
                ...
                string selectedFamilyTypeItemUrl = workItemData["selectedFamilyTypeItemUrl"].Value<string>();
                string selectedFamilyTypeFolderUrl = workItemData["selectedFamilyTypeFolderUrl"].Value<string>();
                string selectedFamilyTypeItemName = workItemData["selectedFamilyTypeItemName"].Value<string>();
                string selectedFamilyTypeInstanceGuid = workItemData["selectedFamilyTypeInstanceGuid"].Value<string>();
                string selectedFamilyTypeName = workItemData["selectedFamilyTypeName"].Value<string>();
                string selectedFamilyTypeCategory = workItemData["selectedFamilyTypeCategory"].Value<string>();

                string selectedFamilyInstanceItemUrl = workItemData["selectedFamilyInstanceItemUrl"].Value<string>();
                string selectedFamilyInstanceFolderUrl = workItemData["selectedFamilyInstanceFolderUrl"].Value<string>();
                string selectedFamilyInstanceItemName = workItemData["selectedFamilyInstanceItemName"].Value<string>();
                string selectedFamilyInstanceGuid = workItemData["selectedFamilyInstanceGuid"].Value<string>();
                string selectedFamilyInstanceTypeName = workItemData["selectedFamilyInstanceTypeName"].Value<string>();
                string selectedFamilyInstanceCategory = workItemData["selectedFamilyInstanceCategory"].Value<string>();
                ...
                string[] outputFolderParams = selectedFamilyInstanceFolderUrl.Split('/');

                string folderId = outputFolderParams[outputFolderParams.Length - 1];
                string projectId = outputFolderParams[outputFolderParams.Length - 3];

                VersionsApi versionApi = new VersionsApi();
                versionApi.Configuration.AccessToken = Credentials.TokenInternal;

                // Revit file download URL
                string[] revitFileParams = selectedFamilyInstanceItemUrl.Split('/');
                ...
                string[] familyFileParams = selectedFamilyTypeItemUrl.Split('/');
                ...
                inputJson.FamilyFileName = selectedFamilyTypeItemName;
                inputJson.FamilyCategory = selectedFamilyInstanceCategory;
                inputJson.FamilyTypeName = selectedFamilyTypeName;
                inputJson.InstanceGuid = selectedFamilyInstanceGuid;
                ...
                // 3. output file
                string outputRVTFileStorageName = selectedFamilyInstanceItemName;
                ...
                // prepare & submit workitem
                string callbackUrl = string.Format("{0}/api/forge/fm_revit/replace_family/callback?id={1}&rvtFileName={2}&rvtFileBucketKey={3}&rvtFileObjectName={4}&rvtStorageId={5}&projectId={6}&folderId={7}", Credentials.GetAppSetting("FORGE_WEBHOOK_URL"), browerConnectionId, outputRVTFileStorageName, rvtFileBucketKey, rvtFileObjectName, (string)storageCreatedRVT.data.id, projectId, folderId);
                ...
                {
                    { "inputRvtFile", inputRvtFileArgument },
                    { "inputRfaFile", inputFamilyFileArgument },
                    { "inputJsonFile",inputJsonArgument },
                    { "resultRvtFile", outputRvtFileArgument },
                    { "onComplete", new XrefTreeArgument { Verb = Verb.Post, Url = callbackUrl } }
                }
        ...
        [HttpPost]
        [Route("api/forge/fm_revit/replace_family/callback")]
        public async Task<IActionResult> OnCallbackReplaceFamily(string id, string rvtFileName, string rvtFileBucketKey, string rvtFileObjectName, string rvtStorageId, string projectId, string folderId, [FromBody] dynamic body)
        {
            try
            {
                await CreateItemVersion(projectId, folderId, rvtFileName, rvtStorageId);
                ...
                await _hubContext.Clients.Client(id).SendAsync("onCompleteReplaceFamily", jsonData);
        ...
        [HttpPost]
        [Route("api/forge/fm_revit/prepare_family")]
        public async Task<IActionResult> PrepareFamily([FromForm] InputData input)
        {
            JObject workItemData = JObject.Parse(input.data);
            ...
            try
            {
                ...
                string selectedFamilyItemUrl = workItemData["selectedFamilyItemUrl"].Value<string>();
                string selectedFamilyFolderUrl = workItemData["selectedFamilyFolderUrl"].Value<string>();
                string selectedFamilyItemName = workItemData["selectedFamilyItemName"].Value<string>();
                ...
                string[] outputFolderParams = selectedFamilyFolderUrl.Split('/');
                ...
                // Family file download URL
                string[] familyFileParams = selectedFamilyItemUrl.Split('/');
                string familyFileProjectIdParam = familyFileParams[1];
                string familyFileVersionIdParam = familyFileParams[2];
                dynamic familyFileVersion = await versionApi.GetVersionAsync(familyFileProjectIdParam, familyFileVersionIdParam);
                string[] familyFileVersionStorageParams = ((string)familyFileVersion.data.relationships.storage.data.id).Split('/');
                ...
                // 3. output file
                string outputRVTFileStorageName = selectedFamilyItemName.Replace("rfa", "rvt");
                ...
                // prepare & submit workitem
                string callbackUrl = string.Format("{0}/api/forge/fm_revit/prepare_family/callback?id={1}&rvtFileName={2}&rvtFileBucketKey={3}&rvtFileObjectName={4}&rvtStorageId={5}&projectId={6}&folderId={7}", Credentials.GetAppSetting("FORGE_WEBHOOK_URL"), browerConnectionId, outputRVTFileStorageName, rvtFileBucketKey, rvtFileObjectName, (string)storageCreatedRVT.data.id, projectId, folderId);
        ...
        [HttpPost]
        [Route("api/forge/fm_revit/prepare_family/callback")]
        public async Task<IActionResult> OnCallbackPrepareFamily(string id, string rvtFileName, string rvtFileBucketKey, string rvtFileObjectName, string rvtStorageId, string projectId, string folderId, [FromBody] dynamic body)
        {
            try
            {
                await CreateItemVersion(projectId, folderId, rvtFileName, rvtStorageId);
                ...
                await _hubContext.Clients.Client(id).SendAsync("onCompletePrepareFamily", jsonData);
    ...
    class OutputData
    {
        ...
        [JsonProperty("rvtFileName")]
        public string RvtFileName { get; set; }

        [JsonProperty("rvtFileBucketKey")]
        public string RvtFileBucketKey { get; set; }

        [JsonProperty("rvtFileObjectName")]
        public string RvtFileObjectName { get; set; }
    }
}
